    ...
    setup() {
      ...
      const tituloDialogoProducto = computed(() => {
        if (esDialogoEdicion.value) {
          return esDialogoPlato.value ? 'Editar plato' : 'Editar bebida'
        }

        return esDialogoPlato.value ? 'Nuevo plato' : 'Nueva bebida'
      })
      ...
      const cargarOpcionesCatalogo = async () => {
        try {
          ...
          opcionesCatalogo.value = {
            categorias: Array.isArray(response.data?.categorias) ? response.data.categorias : [],
      ...
      const obtenerIdsTodasGuarniciones = () => {
        return opcionesCatalogo.value.guarniciones.map((guarnicion) => guarnicion.id_guarnicion)
      }
      ...
      const resetFormProducto = (tipoProducto) => {
        ...
        formProducto.value = {
          ...
          modo_control: tipoProducto === 'PLATO' ? 'PLATO_CARNE' : 'BEBIDA_INVENTARIO',

          guarniciones: tipoProducto === 'PLATO' ? obtenerIdsTodasGuarniciones() : [],
      ...
      const detectarModoControlProducto = (producto) => {
        ...
        if (prioridadStock === 'PRODUCCION_DIARIA' || Boolean(producto?.consume_carne)) {
          return 'PLATO_CARNE'
        }
      ...
      const abrirDialogoEditarProducto = async (producto) => {
        const tipoProducto = String(producto?.tipo_producto || 'PLATO').toUpperCase()
        ...
        formProducto.value = {
          ...
          guarniciones: Array.isArray(producto.guarniciones)
            ? producto.guarniciones.map((guarnicion) => guarnicion.id_guarnicion)
            : [],
      ...
      const construirFormDataProducto = () => {
        ...
        if (esDialogoEdicion.value) {
          formData.append('eliminar_imagen', form.eliminar_imagen ? '1' : '0')
        }
        ...
        if (esDialogoPlato.value && esPlatoConCarne.value) {
          ...
          if (Number(form.consumo_chancho || 0) > 0) {
            formData.append('consumos_carne[CHANCHO]', Number(form.consumo_chancho))
          }

          if (Number(form.consumo_pollo || 0) > 0) {
            formData.append('consumos_carne[POLLO]', Number(form.consumo_pollo))
          }
      ...
      const guardarProductoCatalogo = async () => {
        ...
        try {
          ...
          if (esDialogoEdicion.value) {
            ...
            await api.post(`/catalogo/productos/${formProducto.value.id_producto}`, formData, {
              headers: {
      ...
      const usaProduccionDiaria = (producto) => {
        ...
        return prioridadStock === 'PRODUCCION_DIARIA' || Boolean(producto?.consume_carne)
      }
      ...
      onMounted(async () => {
        await Promise.all([cargarProductos(), cargarOpcionesCatalogo()])
      })
