...
class MovimientoCarneController extends Controller
{
    ...
    {
        $user = User::with('empleado')->findOrFail($request->user()->id);

        $idSucursal = $this->movimientoCarneService
            ->obtenerSucursalUsuario($user);

        $jornada = $this->movimientoCarneService
        ...
        $movimientos = MovimientoCarne::query()
            ...
            ->when(
                ...
                fn ($query) => $query->where(
                    'tipo_movimiento',
                    strtoupper($request->string('tipo_movimiento')->toString())
                )
            )
            ->when(
                ...
                fn ($query) => $query->where(
                    'motivo',
                    strtoupper($request->string('motivo')->toString())
                )
            )
            ->orderByDesc('id_movimiento_carne')
            ->paginate(20);

        return response()->json([
            'message' => 'Movimientos de carne obtenidos correctamente.',
            'jornada' => [
    ...
    public function store(RegistrarMovimientoCarneRequest $request)
    {
        $user = User::with('empleado')->findOrFail($request->user()->id);

        $movimiento = $this->movimientoCarneService->registrarManual(
            $user,
            $request->validated()
        );

        return response()->json([
            'message' => 'Movimiento de carne registrado correctamente.',
